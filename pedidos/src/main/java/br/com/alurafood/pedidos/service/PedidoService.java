...
public class PedidoService {
    ...
    public PedidoDto atualizaStatus(Long id, StatusDto dto) {
         ...
         Optional<Pedido> pedido = repository.porIdComItens(id);

        if (pedido.isEmpty()) {
            throw new EntityNotFoundException();
        }

        pedido.get().setStatus(dto.getStatus());
        repository.atualizaStatus(dto.getStatus(), pedido.get());
        return modelMapper.map(pedido, PedidoDto.class);
    }
    ...
    public void aprovaPagamentoPedido(Long id) {
        ...
        repository.atualizaStatus(Status.PAGO, pedido.get());
    }
}
